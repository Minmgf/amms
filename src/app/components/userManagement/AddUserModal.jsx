...
export default function AddUserModal({ isOpen, onClose, onUserCreated }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        setError(null);
        const [docTypesResponse, genderTypesResponse, roleTypesResponse] =
          await Promise.all([
            getDocumentTypes(),
            getGenderTypes(),
            getRoleTypes(),
        ...
        if (
          docTypesResponse.success &&
          genderTypesResponse.success &&
          roleTypesResponse.success
        ) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const userData = {
        ...
        type_document_id:
          formData.type_document_id?.value || formData.type_document_id,
      ...
      if (err.message === "No hay token disponible") {
        errorMessage =
          "Error de autenticación: No hay token disponible. Por favor, inicie sesión nuevamente.";
      } else if (err.message === "Token expirado") {
        errorMessage =
          "Error de autenticación: Token expirado. Por favor, inicie sesión nuevamente.";
      } else if (err.message === "Token no válido") {
        errorMessage =
          "Error de autenticación: Token no válido. Por favor, inicie sesión nuevamente.";
      } else if (err.response?.status === 401) {
        errorMessage =
          "Error de autenticación: Credenciales inválidas. Por favor, inicie sesión nuevamente.";
  ...
  const handleAddRole = () => {
    if (
      formData.role &&
      !formData.roles.some((r) => r.value === formData.role.value)
    ) {
      setFormData((prev) => ({
        ...prev,
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-lg shadow-xl z-[50] w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-white">
          ...
          <div className="flex justify-between items-center px-8 pt-6 pb-4">
            ...
            <button
              onClick={onClose}
              className="text-black text-2xl font-bold hover:text-red-500"
            >
              &times;
            </button>
          </div>
          <form onSubmit={handleSubmit} className="px-8 pb-8 pt-2">
            ...
            <div className="grid grid-cols-3 gap-4 mb-4">
              ...
              <div>
                ...
                <Select
                  ...
                  className={`react-select-container ${
                    hasSubmitted && !formData.type_document_id
                      ? "border-red-600"
                      : "border-gray-300"
                  }`}
                  isSearchable
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full rounded border px-3 py-2 bg-white text-black text-base ${
                    hasSubmitted && !formData.document_number
                      ? "border-red-600"
                      : "border-gray-300"
                  }`}
                  placeholder=""
              ...
              <div>
                ...
                <Select
                  ...
                  onChange={(option) =>
                    setFormData((prev) => ({ ...prev, gender_id: option }))
                  }
                  ...
                  placeholder=""
                  className={`react-select-container ${
                    hasSubmitted && !formData.gender_id
                      ? "border-red-600"
                      : "border-gray-300"
                  }`}
                  isSearchable
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      date_issuance_document: e.target.value,
                    }))
                  }
            ...
            {/* Roles seleccionados */}
            <div className="mb-6">
              <div className="bg-gray-100 rounded-t px-4 py-2 font-semibold text-black border-b border-gray-300">
                Roles Seleccionados
              </div>
              <div className="bg-gray-100 rounded-b px-4 py-4 min-h-[60px]">
                {formData.roles.length === 0 ? (
                  <p className="text-gray-500 text-sm">
                    No se han agregado roles aún. Selecciona un elemento del
                    menú desplegable anterior y haz clic en "Agregar".
                  </p>
                ) : (
                  <div className="flex flex-wrap gap-2">
                    {formData.roles.map((role, index) => (
                      <span
                        key={index}
                        className="px-3 py-1 bg-white border border-gray-300 text-black rounded-full text-sm flex items-center gap-2"
                      >
                        {role.label}
                        <button
                          type="button"
