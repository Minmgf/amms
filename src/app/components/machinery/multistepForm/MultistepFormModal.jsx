...
import { getActiveMachinery, getActiveMachine, getModelsByBrandId, getMachineryBrands, registerGeneralData, registerInfoTracker, getMaintenanceTypes, getDistanceUnits, getTenureTypes, getUseStates, registerUsageInfo, confirmMachineryRegistration } from "@/services/machineryService";
...
export default function MultiStepFormModal({ isOpen, onClose }) {
  ...
  const onSubmit = (data) => {
    ...
    if (step === steps.length - 1) {
      ...
      // Ahora confirmamos el registro completo de la maquinaria
          ...
          <form onSubmit={methods.handleSubmit(onSubmit)} className="p-3 sm:p-6 md:p-8 lg:p-theme-xl">
            ...
            <div className="flex flex-row justify-between items-center mt-6 sm:mt-theme-xl pt-4 sm:pt-theme-lg">
              ...
              {isLastStep ? (
                ...
                >
                  {isConfirmingRegistration ? 'Confirmando registro...' : isSubmittingStep ? 'Guardando...' : 'Guardar'}
                </button>
              ) : (
                <button
                  type="button"
